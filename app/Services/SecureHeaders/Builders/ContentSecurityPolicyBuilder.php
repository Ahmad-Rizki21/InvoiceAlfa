...
namespace App\Services\SecureHeaders\Builders;
...
final class ContentSecurityPolicyBuilder extends Builder
{
    ...
    protected function directives(): string
    {
        ...
        $isRunningVite = Vite::isRunningHot();
        $isHotHttps = false;
        $hotUrl = null;
        $hotWsUrl = null;
        $hotWssUrl = null;
        ...
        if ($isRunningVite) {
            ...
            if ($hotUrl) {
                $isHotHttps = strpos($hotUrl, 'https:') !== false;
                $hotWsUrl = preg_replace('~https?~', '', $hotUrl);
                $hotWssUrl = 'wss' . $hotWsUrl;
                $hotWsUrl = 'ws' . $hotWsUrl;
            }
        }
        ...
        foreach ($this->config as $name => $config) {
            ...
            if ($isRunningVite && $hotUrl) {
                if (! array_key_exists('allow', $config)) {
                    $config['allow'] = [];
                }

                if (! is_array($config['allow'])) {
                    $config['allow'] = [$config['allow']];
                }

                if ($name === 'connect-src') {
                    $config['allow'][] = $isHotHttps ? $hotWssUrl : $hotWsUrl;
                } else if (in_array($name, ['font-src', 'style-src'])) {
                    $config['allow'][] = $hotUrl;
                }
            }
