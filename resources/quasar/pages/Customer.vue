  ...
  <q-page class="page-customers">
    ...
    <div class="page-header">
      ...
      <q-btn
        v-if="$auth.can('create.customer')"
        color="primary"
        class="q-btn-lg btn-page-customers-create"
        icon="add"
        @click="onFormEntryCreate"
      >
        <span class="q-ml-xs">{{ $t('New {entity}', { entity: $t('Customer') }) }}</span>
      </q-btn>
    </div>
          ...
          <template #body-cell-action="{ row }">
            <q-td class="text-right table-cell-action">
              <q-btn
                ...
                padding="xs"
                icon="visibility"
                class="btn-datatable-view-customer"
              ...
              <q-btn
                v-if="$auth.can('edit.customer')"
                unelevated
                color="default"
                size="sm"
                padding="xs"
                icon="lock_open"
                class="q-ml-xs btn-datatable-update-password-customer"
              ...
              <q-btn
                v-if="$auth.can('delete.customer')"
                unelevated
                color="default"
                size="sm"
                padding="xs"
                icon="delete"
                class="q-ml-xs btn-datatable-delete-customer"
...
import FormEntry from './Customer/FormEntry'
import FormPassword from './Customer/FormPassword'
...
export default {
  ...
  meta() {
    return {
      title: this.$t('Customers') + ' - ' + this.$t('Web Console')
    }
  },
  breadcrumbs() {
    return [
  ...
  async mounted() {
    ...
    // if (!this.$store.getters['tourGuide/finishedGroups'].customers) {
    //   this.$tourGuide.open('customers')
    // }
  },
  methods: {
    async onRequest(props) {
      ...
      try {
        const { data } = await this.$api.get('/v1/customers', { params })

        if (data.status === 'success') {
          this.entries = data.data.customers
    ...
    onDelete(row) {
      ...
      this.$q.dialog({
        ...
        message: this.$t('Are you sure want to delete this {entity}?', { entity: this.$t('customer') }),
        cancel: {
          label: this.$t('Cancel'),
          color: 'dark',
          flat: true
        ...
        try {
          let { data } = await this.$api.delete(`/v1/customers/${row.id}`);
          ...
          if (data.message) {
            ...
            if (data.status === 'success') {
              this.$q.notify({ message: this.$t('{entity} deleted', { entity: this.$t('Customer') }) })
              this.onRequest()
            } else {
              this.$t('Failed to delete {entity}', { entity: this.$t('customer') })
            }
          }
        } catch (err) {
          this.$q.notify(err);
