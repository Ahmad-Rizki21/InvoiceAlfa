<template>
  <q-page class="page-stores page-store-single page-store-show">
...
export default {
  ...
  meta() {
    return {
      title: this.$t('Store') + ' - ' + this.$t('Web Console')
    }
  },
  breadcrumbs() {
    const breadcrumbs = [
  ...
  methods: {
    async onRequest(props) {
      ...
      try {
        const { data } = await this.$api.get(`/v1/stores/${props.id}`, { params })

        if (data.status === 'success') {
          this.entry = data.data.store
        }
      } catch (err) {
        this.$q.notify(err)
      }
