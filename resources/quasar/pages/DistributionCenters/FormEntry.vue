  ...
  <q-card class="form-entry q-pa-sm" :class="{ readonly }">
    <q-form ref="form" class="q-gutter-md" greedy @submit.prevent="onSubmit">
      <div v-if="!readonly" class="text-h6 q-px-md q-pt-sm">
        {{ $t(`${isCreate ? 'Create' : 'Update'} {entity}`, { entity: $t('Customer') }) }}
      </div>
        ...
        <div class="row q-col-gutter-sm">
          ...
          <div class="col-xs-12 col-sm-5 col-md-4">
            <q-input
              ...
              mask="##/##/####"
              :hint="$utils.isDateValid(formEntry.online_at) ? undefined : $t('Valid format: DD/MM/YYYYY')"
            >
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        const endpoint = entry.id ? `/v1/customers/${entry.id}` : '/v1/customers';
        ...
        if (data.message) {
          ...
          if (data.status === 'success') {
            this.$q.notify({ message: this.$t('{entity} saved', { entity: this.$t('Customer') }) })
          } else {
            this.$t('Failed to save {entity}', { entity: this.$t('customer') })
          }
        }
      } catch (err) {
        if (err.validation) {
