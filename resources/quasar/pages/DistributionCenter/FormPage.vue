<template>
  <div class="form-page q-pa-sm q-gutter-md">
        ...
        <q-card-section>
          <div class="row">
            <div :class="{ 'col-xs-12': !readonly, 'col-xs-11': readonly }">
              ...
              <div class="q-mb-md text-subtitle text-weight-bold">{{ $t('Login Credentials') }}</div>
    ...
    <div v-if="!isCreate" class="table-relation">
      <div class="table-relation-header">
        <q-tabs
          v-model="childTab"
          shrink
          no-caps
        >
          <q-tab name="franchise" :label="$t('Franchise')" />
          <q-tab name="store" :label="$t('Store')" />
        </q-tabs>
        ...
        <q-btn
          ...
          size="sm"
          icon="add"
          @click="onTableRelationFranchiseCreate"
        >
          <span class="q-ml-xs">{{ $t('New {entity}', { entity: $t('Franchise') }) }}</span>
        </q-btn>
        <q-btn
          v-if="childTab === 'store' && $auth.can('create.store')"
          color="primary"
          class="btn-page-store-create"
          size="sm"
          icon="add"
          @click="onTableRelationStoreCreate"
        >
...
const DEFAULT_FORM_ENTRY = {
  ...
  npwp: null,
  transfer_to_virtual_account_bank_name: null,
  transfer_to_virtual_account_number: null,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        npwp: [
          v => !v || (!!v && String(v).replace(/[^\d]/g, '').length === 15) || this.$t('{field} must be {length} characters', { field: this.$t('NPWP'), length: 15 })
        ]
      },
  ...
  computed: {
    ...
    dialogImportProps() {
      ...
      return {
        importType: this.$constant.import_type.Store,
        templateUrl: this.$q.lang.isoName === 'id' ? '/templates/template-toko.xlsx' : '/templates/store-template.xlsx'
      }
    }
  },
  watch: {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      if (entry.npwp) {
        entry.npwp = String(entry.npwp).replace(/[^\d]/g, '')
      }
